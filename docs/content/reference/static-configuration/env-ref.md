...
`TRAEFIK_METRICS_OPENTELEMETRY_HTTP`:  
HTTP specific configuration for the OpenTelemetry collector. (Default: ```false```)

`TRAEFIK_METRICS_OPENTELEMETRY_HTTP_URLPATH`:  
Override the default URL path used for sending traces. (Default: ```/v1/metrics```)
...
`TRAEFIK_TRACING_OPENTELEMETRY_HTTP`:  
HTTP specific configuration for the OpenTelemetry collector. (Default: ```false```)

`TRAEFIK_TRACING_OPENTELEMETRY_HTTP_URLPATH`:  
Override the default URL path used for sending traces. (Default: ```/v1/traces```)
