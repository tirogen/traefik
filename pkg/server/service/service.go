...
func (m *Manager) getFailoverServiceHandler(ctx context.Context, serviceName string, config *dynamic.Failover) (http.Handler, error) {
	...
	failoverHandler, err := m.BuildHTTP(ctx, config.Failover)
	if err != nil {
		return nil, err
	}
	...
	failoverUpdater, ok := failoverHandler.(healthcheck.StatusUpdater)
	if !ok {
		return nil, fmt.Errorf("child service %v of %v not a healthcheck.StatusUpdater (%T)", config.Failover, serviceName, serviceHandler)
	}

	if err := failoverUpdater.RegisterStatusUpdater(func(up bool) {
		f.SetFailoverHandlerStatus(ctx, up)
	}); err != nil {
		return nil, fmt.Errorf("cannot register %v as updater for %v: %w", config.Failover, serviceName, err)
	}
